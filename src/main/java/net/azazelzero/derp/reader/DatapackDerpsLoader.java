...
//                }
    ...
    private void buildDerp(JsonObject jsonFile){
        ...
        if(jsonFile.has("Id"))             derp.name = derp.grab("Id", jsonFile).getAsString();
        if(jsonFile.has("DisplayName"))    derp.displayName = derp.grab("DisplayName", jsonFile).getAsString();
        if(jsonFile.has("Icon"))           derp.Icon = derp.grab("Icon", jsonFile).getAsString();
        if(jsonFile.has("UiBackground"))   derp.Texture = derp.grab("UiBackground", jsonFile).getAsString();
        if(jsonFile.has("Description"))    derp.Description = derp.grab("Description", jsonFile).getAsString();
        ...
        AtomicInteger skillIndex = new AtomicInteger();
        if (jsonFile.get("Panels") != null&&jsonFile.get("Panels").isJsonArray()) {
            JsonArray array= jsonFile.get("Panels").getAsJsonArray();
            derp.Panels=new DatRequirement[array.size()][];
            for (int i = 0; i < array.size(); i++) {
                derp.Panels[i]=requirementsAdd(array.get(i).getAsJsonObject());
            }
        } else derp.Panels = new DatRequirement[8][];
...
//        if(!(ServerLifecycleHooks==null)) PacketHandler.INSTANCE.send(PacketDistributor.PLAYER.with(() -> ServerLifecycleHooks.getCurrentServer().getPlayerList().getPlayerByName("Dev")),new Message(new Packet("pain")));
    ...
    private @Nullable DatRequirement[] requirementsAdd(JsonObject skillValue){
        AtomicInteger RequirementIndex = new AtomicInteger();
        boolean isArray = skillValue.get("Requirements") != null;
        DatRequirement[] Requirements;
        if (isArray) {
            Requirements = new DatRequirement[skillValue.get("Requirements").getAsJsonArray().size()];
            skillValue.get("Requirements").getAsJsonArray().forEach((requirements) -> {
                String name = requirements.getAsJsonObject().get("RequirementType").getAsString();
                if (Objects.equals(name, "")) {
                    name = "no:type";
                ...
                RegistryObject<RequirementRegistryEntry> val2 = RegistryObject.create(new ResourceLocation(name.substring(0, colon), name.substring(colon + 1)), DatRequirementRegistry.location, "derp");
                if (val2.isPresent()) {
                    Requirements[RequirementIndex.get()] = val2.get().DatFunction();
                    Requirements[RequirementIndex.get()].setParameters(requirements.getAsJsonObject());
