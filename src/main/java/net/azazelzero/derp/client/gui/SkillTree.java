...
//        System.out.println("b:"+b);
        ...
        if(variableCarryover.get(3)==1)variableCarryover.set(4,variableCarryover.get(4)+(int)c);
...
//        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
    ...
    public void renderSkillsHover(PoseStack matrices, int width, int height,int mouseX,int mouseY){
        ...
        for (int x = 0; x < panel.length; x++) {
            for (int y = 0; y < panel[x].length; y++) {
                if(!scrollLock) scrollLock=renderSkillHover(matrices,width+11+(x*23),height+23+(y*23),panel[x][y],mouseX,mouseY);
            }
        }
    ...
    private boolean renderSkillHover(PoseStack matrices, int x, int y, DAT dat, int mouseX, int mouseY) {
        ...
        if(!hover&&variableCarryover.get(5)==x) hover=isMouseInBetween(x+18,x+18+82,y,y+71,mouseX,mouseY);
        if(hover){
            ...
            if((Lines.size()*8)+yCounter<112&&(Lines.size()*8)>120)yCounter=(((Lines.size()*8)-112)*-1);
            if(yCounter>10)yCounter=10;
            if (Lines.size()*8<120)yCounter=10;
...
//            blit(matrices,x+18,y,9,0.6835f,0.7226f,84,12,256,256);
    ...
    public void renderSkills(PoseStack matrices, int width, int height){
        ...
        for (int x = 0; x < panel.length; x++) {
            for (int y = 0; y < panel[x].length; y++) {
                renderSkill(matrices,width+11+(x*23),height+23+(y*23),panel[x][y]);
            }
        }
    }
